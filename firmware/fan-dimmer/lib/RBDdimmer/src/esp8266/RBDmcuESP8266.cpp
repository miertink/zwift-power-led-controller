...
// TRIAC gate pulse width, in ~87us timer ticks (was 1 = ~87us). Widened to give an
...
// half-cycles ("trancos") specifically at high duty.
int pulseWidth = 4;
...
{
	if (!zcDiagRecordSample()) {
		return; // debounced: electrical bounce/noise, not a genuine zero-cross
	}
