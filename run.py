...
def setup_mqtt():
    ...
    try:
        ...
        logger.info(
            f"MQTT client successfully connected to {MQTT_HOST_NAME}")
        return mqtt_client
    except:
        logger.error(
            f"MQTT client could not connect to {MQTT_HOST_NAME}, check settings.py")
        exit()
...
def main():
    ...
    try:
        ...
        logger.error(
            f'Error retrieving user profile, please check username, password and PLAYER_ID in settings.py')
        exit()
